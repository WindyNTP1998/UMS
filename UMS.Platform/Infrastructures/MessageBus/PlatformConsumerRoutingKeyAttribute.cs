...
///     If not defined, consumer will bind to default free format message routing key from
///     <inheritdoc cref="PlatformBusMessageRoutingKey.BuildDefaultRoutingKey" />
/// </summary>
...
public class PlatformConsumerRoutingKeyAttribute : Attribute
{
    ...
    ///     Pattern value support <see cref="PlatformBusMessageRoutingKey.MatchAllSingleGroupLevelChar" /> to match startWith
    ///     (*XXX), endWith (XXX*) and contains (*XXX*)
    /// </summary>
    ...
    /// </param>
    public PlatformConsumerRoutingKeyAttribute(string messageGroup,
        ...
        string messageAction = null)
    {
        ...
        MessageType = PlatformBusMessageRoutingKey.AutoFixKeyPart(messageType) ??
                      PlatformBusMessageRoutingKey.MatchAllSingleGroupLevelChar;
        MessageAction = PlatformBusMessageRoutingKey.AutoFixKeyPart(messageAction) ??
                        PlatformBusMessageRoutingKey.MatchAllSingleGroupLevelChar;
    ...
    /// </summary>
    /// <param name="combinedKeyPattern">
    ///     Combined key pattern string represent for
    ///     {messageGroup}.{producerContext}.{messageType}.{messageAction}
    /// </param>
    ...
    public bool IsMatchMessageRoutingKey(string messageRoutingKey)
    {
        if (CustomRoutingKey.IsNotNullOrEmpty())
            return PlatformBusMessageRoutingKey.IsMatchRoutingKeyPattern(CustomRoutingKey,
                       messageRoutingKey) ||
                   PlatformBusMessageRoutingKey.IsMatchRoutingKeyPattern(
                       PlatformBusMessageRoutingKey.BuildCombinedStringKey(MessageGroup,
                           CustomRoutingKey),
                       messageRoutingKey);
    ...
    {
        var patternRoutingKey = PlatformBusMessageRoutingKey.New(MessageGroup,
            ProducerContext,
            MessageType,
            MessageAction);
