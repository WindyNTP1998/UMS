...
public class PlatformBusMessageRoutingKey : IEqualityComparer<PlatformBusMessageRoutingKey>
{
    ...
    /// </summary>
    public string CombinedStringKey => BuildCombinedStringKey(MessageGroup, ProducerContext, MessageType,
        MessageAction);
    ...
    public static PlatformBusMessageRoutingKey BuildDefaultRoutingKey(Type messageType,
        string producerContext = MatchAllSingleGroupLevelChar)
    {
        var messagePayloadTypeName =
            messageType.FindMatchedGenericType(typeof(IPlatformWithPayloadBusMessage<>))?.GetGenericArguments()[0]
    ...
    public static string BuildCombinedStringKey(string messageGroup,
    ...
    public static PlatformBusMessageRoutingKey New(string messageGroup,
        string producerContext,
        string messageType,
        string messageAction = null)
    {
    ...
    public static PlatformBusMessageRoutingKey New(string combinedKey)
    {
        ...
        return New(combinedPatternParts.ElementAtOrDefault(0),
            combinedPatternParts.ElementAtOrDefault(1),
            combinedPatternParts.ElementAtOrDefault(2),
            combinedPatternParts.ElementAtOrDefault(3));
    }

    public static bool MatchPattern(string pattern, string value)
    {
        if (pattern == MatchAllSingleGroupLevelChar || pattern == value) return true;
    ...
    {
        return new PlatformSingleValidator<PlatformBusMessageRoutingKey, string>(keyPartSelector,
            ruleBuilder =>
            {
                ...
                if (allowNull) ruleBuilderOptions.When(key => keyPartSelector.Compile()(key) != null);
            });
    }

    public static PlatformValidator<PlatformBusMessageRoutingKey> Validator(bool forMatchingPattern = false)
    {
        return PlatformValidator<PlatformBusMessageRoutingKey>.Create(KeyPartValidator(p => p.MessageGroup),
            KeyPartValidator(p => p.ProducerContext, forMatchingPattern),
            KeyPartValidator(p => p.MessageType, forMatchingPattern),
            KeyPartValidator(p => p.MessageAction, forMatchingPattern, true));
    }
    ...
    {
        if (ToString() == routingKey) return true;
        ...
        Func<PlatformValidationResult, Exception> exceptionProvider = null)
    {
        ...
        validationResult.EnsureValid(p => exceptionProvider != null
            ? exceptionProvider(p)
