...
public class PlatformCollectionCacheKeyProvider : PlatformContextCacheKeyProvider
{
    ...
    {
        return this.Ensure(p => p.Context.IsNotNullOrEmpty() && p.Collection.IsNotNullOrEmpty(),
            () => new Exception("Context and Collection must be not null"));
    }
}

public abstract class PlatformCollectionCacheKeyProvider<TFixedImplementationProvider>
    : PlatformCollectionCacheKeyProvider
