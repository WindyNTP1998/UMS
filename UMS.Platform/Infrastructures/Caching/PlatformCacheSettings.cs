...
public class PlatformCacheSettings
{
    ...
    public async Task<T> ExecuteWithSlowWarning<T>(Func<Task<T>> executeFn, Func<ILogger> loggerFactory,
        bool forSetData = false)
    {
        if (SlowWarning.IsEnabled)
        {
            ...
            if (queryElapsedTime.TotalMilliseconds >= SlowWarning.GetSlowQueryMillisecondsThreshold(forSetData))
                loggerFactory()
                    .Log(SlowWarning.IsLogWarningAsError ? LogLevel.Error : LogLevel.Warning,
