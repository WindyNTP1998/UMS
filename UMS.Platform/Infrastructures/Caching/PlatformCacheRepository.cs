...
public interface IPlatformCacheRepository
{
    ...
    Task SetAsync<T>(PlatformCacheKey cacheKey,
        T value,
    ...
    /// <summary>
    ///     Return cache from request function if exist. If not, call request function to get data, cache the data and return
    ///     it.
    /// </summary>
    ...
    /// <summary>
    ///     Return default cache entry options value. This could be config when register module, override
    ///     <see cref="PlatformCachingModule.ConfigCacheSettings" />
    /// </summary>
...
public abstract class PlatformCacheRepository : IPlatformCacheRepository
{
    ...
    public abstract Task SetAsync<T>(PlatformCacheKey cacheKey,
        T value,
        PlatformCacheEntryOptions cacheOptions = null,
        CancellationToken token = default);
    ...
    public async Task SetAsync<T>(PlatformCacheKey cacheKey,
        T value,
        double? absoluteExpirationInSeconds = null,
        CancellationToken token = default)
    {
    ...
    public abstract Task RemoveAsync(Func<PlatformCacheKey, bool> cacheKeyPredicate,
        CancellationToken token = default);

    public async Task RemoveCollectionAsync<TCollectionCacheKeyProvider>(CancellationToken token = default)
        where TCollectionCacheKeyProvider : PlatformCollectionCacheKeyProvider
    {
        await RemoveAsync(serviceProvider.GetService<TCollectionCacheKeyProvider>()!.MatchCollectionKeyPredicate(),
            token);
    }

    public async Task<TData> CacheRequestAsync<TData>(Func<Task<TData>> request,
        PlatformCacheKey cacheKey,
        PlatformCacheEntryOptions cacheOptions = null,
        CancellationToken token = default)
    {
        ...
        return cachedDataResult.IsValid && cachedDataResult.Value != null
            ? cachedDataResult.Value
            : await RequestAndCacheNewData();
    ...
    protected async Task<PlatformValidationResult<T>> TryGetAsync<T>(PlatformCacheKey cacheKey,
        CancellationToken token = default)
    {
        try
        {
    ...
    protected async Task TrySetAsync<T>(PlatformCacheKey cacheKey,
        T value,
        PlatformCacheEntryOptions cacheOptions = null,
        CancellationToken token = default)
    {
        try
        {
            await SetAsync(cacheKey, value, cacheOptions,
                token);
        }
        catch (Exception ex)
        {
    ...
    protected async Task<ConcurrentDictionary<PlatformCacheKey, object>> LoadGlobalAllRequestCachedKeys()
    {
        try
        {
            return await GetAsync<List<PlatformCacheKey>>(GetGlobalAllRequestCachedKeysCacheKey())
    ...
    protected DistributedCacheEntryOptions MapToDistributedCacheEntryOptions(PlatformCacheEntryOptions options)
    {
        return new DistributedCacheEntryOptions
        {
            AbsoluteExpirationRelativeToNow = options?.AbsoluteExpirationRelativeToNow() ??
                                              CacheSettings.DefaultCacheEntryOptions.AbsoluteExpirationRelativeToNow(),
            SlidingExpiration = options?.SlidingExpiration() ??
                                CacheSettings.DefaultCacheEntryOptions.SlidingExpiration()
        };
    }
}