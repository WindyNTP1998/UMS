...
namespace UMS.Platform.Infrastructures.Caching.BuiltInCacheRepositories;
...
public class PlatformMemoryCacheRepository : PlatformCacheRepository, IPlatformMemoryCacheRepository
{
    ...
    public override async Task SetAsync<T>(PlatformCacheKey cacheKey,
        T value,
        PlatformCacheEntryOptions cacheOptions = null,
        CancellationToken token = default)
    {
        await CacheSettings.ExecuteWithSlowWarning(async () =>
            {
                await Util.TaskRunner.WhenAll(SetToMemoryDistributedCacheAsync(cacheKey, value, cacheOptions,
                        token),
    ...
    public override async Task RemoveAsync(Func<PlatformCacheKey, bool> cacheKeyPredicate,
        CancellationToken token = default)
    {
        ...
        if (globalMatchedKeys.Any())
        {
            ...
            clonedMatchedKeys.ForEach(matchedKey =>
            {
    ...
    protected async Task UpdateGlobalCachedKeys(
        Action<ConcurrentDictionary<PlatformCacheKey, object>> updateCachedKeysAction)
    {
    ...
    private async Task SetGlobalCachedKeysAsync(ConcurrentDictionary<PlatformCacheKey, object> globalCachedKeys)
    {
        ...
        await SetToMemoryDistributedCacheAsync(cacheKey,
            cacheValue,
    ...
    private async Task SetToMemoryDistributedCacheAsync<T>(PlatformCacheKey cacheKey,
        T value,
        PlatformCacheEntryOptions cacheOptions = null,
        CancellationToken token = default)
    {
        try
        {
            await memoryDistributedCache.SetAsync(cacheKey,
