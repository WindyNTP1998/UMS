...
using UMS.Platform.Application.Cqrs.Events.InboxSupport;
...
public interface IPlatformCqrsEventApplicationHandler<in TEvent>
    : IPlatformCqrsEventApplicationHandler, IPlatformCqrsEventHandler<TEvent>
...
public abstract class PlatformCqrsEventApplicationHandler<TEvent>
    : PlatformCqrsEventHandler<TEvent>, IPlatformCqrsEventApplicationHandler<TEvent>
    where TEvent : PlatformCqrsEvent, new()
{
    protected readonly IServiceProvider ServiceProvider;
    protected readonly IUnitOfWorkManager UnitOfWorkManager;

    public PlatformCqrsEventApplicationHandler(ILoggerFactory loggerFactory,
        ...
        IPlatformRootServiceProvider rootServiceProvider) : base(loggerFactory, rootServiceProvider)
    {
        ...
        IsInjectingApplicationBusMessageProducer =
            GetType().IsUsingGivenTypeViaConstructor<IPlatformApplicationBusMessageProducer>();
    ...
    /// <summary>
    ///     Default return True. When True, Support for store cqrs event handler as inbox if inbox bus message is enabled in
    ///     persistence module
    /// </summary>
    ...
    protected override async Task DoHandle(TEvent @event, CancellationToken cancellationToken,
        bool couldRunInBackgroundThread)
    {
        ...
        var hasInboxMessageRepository =
            RootServiceProvider.CheckHasRegisteredScopedService<IPlatformInboxBusMessageRepository>();
            ...
            !@event.MustWaitHandlerExecutionFinishedImmediately(GetType()))
        {
            ...
            if (@event is IPlatformUowEvent && eventSourceUow != null && !eventSourceUow.IsPseudoTransactionUow())
            {
                ...
                await PlatformInboxMessageBusConsumerHelper.HandleExecutingInboxConsumerAsync(RootServiceProvider,
                    ...
                    inboxConfig,
                    CqrsEventInboxBusMessage(@event, GetType(), applicationSettingContext,
                        currentBusMessageIdentity),
                    PlatformBusMessageRoutingKey.BuildDefaultRoutingKey(typeof(TEvent),
                        applicationSettingContext.ApplicationName),
                    CreateGlobalLogger,
                    PlatformInboxBusMessage.DefaultRetryProcessFailedMessageInSecondsUnit,
                    AllowHandleInBackgroundThread(@event),
                    null,
                    eventSourceUow,
                    AutoDeleteProcessedInboxEventMessage,
                    cancellationToken);
            }
            else
            {
                    ...
                    IPlatformApplicationSettingContext applicationSettingContext) =>
                {
                    var consumerInstance =
                        serviceProvider.GetRequiredService<PlatformCqrsEventInboxBusMessageConsumer>();
                    var inboxConfig = serviceProvider.GetRequiredService<PlatformInboxConfig>();
    ...
    protected virtual PlatformBusMessage<PlatformCqrsEventBusMessagePayload> CqrsEventInboxBusMessage(TEvent @event,
        Type eventHandlerType,
        IPlatformApplicationSettingContext applicationSettingContext,
        PlatformBusMessageIdentity currentBusMessageIdentity)
    {
        return PlatformBusMessage<PlatformCqrsEventBusMessagePayload>
            .New<PlatformBusMessage<PlatformCqrsEventBusMessagePayload>>($"{@event.Id}-{eventHandlerType.Name}",
                PlatformCqrsEventBusMessagePayload.New(@event, eventHandlerType.FullName),
                currentBusMessageIdentity,
                applicationSettingContext.ApplicationName,
                nameof(PlatformCqrsEvent),
                @event.EventAction,
    ...
    public virtual PlatformBusMessageIdentity BuildCurrentBusMessageIdentity(
