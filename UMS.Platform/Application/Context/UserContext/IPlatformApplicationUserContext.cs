...
using UMS.Platform.Common.RequestContext;

namespace UMS.Platform.Application.Context.UserContext;

/// <summary>
///     This is the current context data in the current local thread from IPlatformApplicationUserContextAccessor. Please
///     never save this context as a property in any Service/Command/Handler.
...
{
    public static IPlatformApplicationUserContext SetValues(this IPlatformApplicationUserContext context,
    ...
    {
        if (contextKey == null) throw new ArgumentNullException(nameof(contextKey));

        if (context is IPlatformApplicationUserContext userContext) return userContext.GetValue<T>(contextKey);
        if (PlatformRequestContextHelper.TryGetValue(context, contextKey, out T item)) return item;
