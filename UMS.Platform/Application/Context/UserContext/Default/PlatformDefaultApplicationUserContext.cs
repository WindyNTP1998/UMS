...
namespace UMS.Platform.Application.Context.UserContext.Default;
...
public class PlatformDefaultApplicationUserContext : IPlatformApplicationUserContext
{
    ...
    public PlatformDefaultApplicationUserContext()
    {
        getValueByGenericTypeMethodInfo =
            GetType()
                .GetMethods()
                .First(p => p.IsGenericMethod && p.Name == nameof(GetValue) && p.GetGenericArguments().Length == 1 &&
                            p.IsPublic);
    }
    ...
    public object GetValue(Type valueType, string contextKey)
    {
        return getValueByGenericTypeMethodInfo
            ...
            .Invoke(this, new object[] { contextKey });
    }
