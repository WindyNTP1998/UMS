...
using UMS.Platform.Domain.Exceptions.Extensions;
using UMS.Platform.Domain.UnitOfWork;
using UMS.Platform.Persistence;
using UMS.Platform.Persistence.DataMigration;

namespace UMS.Platform.Application.Persistence;
...
public interface IPlatformDbContext : IDisposable
{
    ...
    public async Task MigrateApplicationDataAsync<TDbContext>(IServiceProvider serviceProvider,
        IPlatformRootServiceProvider rootServiceProvider) where TDbContext : class, IPlatformDbContext<TDbContext>
    {
        ...
        await PlatformDataMigrationExecutor<TDbContext>
            .GetCanExecuteDataMigrationExecutors(GetType().Assembly, serviceProvider,
                ApplicationDataMigrationHistoryQuery)
            .ForEachAsync(async migrationExecution =>
            {
                try
                {
                    if (migrationExecution.AllowRunInBackgroundThread)
                        ExecuteDataMigrationInBackgroundThread(rootServiceProvider, migrationExecution, GetType(),
                            Logger);
                    else
        ...
        Util.TaskRunner.QueueActionInBackground(async () =>
            {
                var currentDbContextPersistenceModule = rootServiceProvider
                    .GetRequiredService(dbContextType.Assembly.GetTypes()
                        .First(p => p.IsAssignableTo(typeof(PlatformPersistenceModule<TDbContext>))))
                ...
                try
                {
                    ...
                    await ExecuteDataMigrationExecutorInNewScope<TDbContext>(rootServiceProvider,
                        migrationExecutionName,
        ...
        try
        {
            await rootServiceProvider.ExecuteInjectScopedAsync(async (IServiceProvider sp) =>
            {
                ...
                await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(
                    () => rootServiceProvider.ExecuteInjectScopedAsync(async (IServiceProvider sp) =>
                    {
                        await dbContext.ExecuteDataMigrationExecutor(
                            PlatformDataMigrationExecutor<TDbContext>.CreateNewInstance(sp,
                                migrationExecutionType));
                    }),
                    ...
                    onRetry: (ex, timeSpan, currentRetry,
                        ctx) =>
                    {
                        dbContext.Logger.LogWarning(ex,
    ...
    public async Task ExecuteDataMigrationExecutor<TDbContext>(
        PlatformDataMigrationExecutor<TDbContext> migrationExecution)
        where TDbContext : class, IPlatformDbContext
    {
        ...
        this.As<TDbContext>()
            .Logger.LogInformation("DataMigration {MigrationExecutionName} FINISHED.", migrationExecution.Name);
    }
        ...
        Func<string>? resultQueryStringBuilder)
    {
        ...
        if (persistenceConfiguration.BadQueryWarning.TotalItemsThresholdWarningEnabled &&
            ...
            HandleLogTooMuchDataInMemoryBadQueryWarning(resultQuery, persistenceConfiguration, logger,
                loggingFullStackTrace, resultQueryStringBuilder);

        var result = await HandleLogSlowQueryBadQueryWarning(getResultFn,
        ...
        static void HandleLogTooMuchDataInMemoryBadQueryWarning(IEnumerable<TSource>? resultQuery,
            IPlatformPersistenceConfiguration persistenceConfiguration,
            ILogger logger,
            ...
            Func<string>? resultQueryStringBuilder)
        {
            ...
            if (queryResultCount >= persistenceConfiguration.BadQueryWarning.TotalItemsThreshold)
                LogTooMuchDataInMemoryBadQueryWarning(queryResultCount, logger, persistenceConfiguration,
                    loggingFullStackTrace, resultQueryStringBuilder);
        }

        static async Task<TResult> HandleLogSlowQueryBadQueryWarning(Func<Task<TResult>> getResultFn,
            IPlatformPersistenceConfiguration persistenceConfiguration,
            ILogger logger,
            string loggingFullStackTrace,
            bool forWriteQuery,
            Func<string>? resultQueryStringBuilder)
        {
            ...
            if (queryElapsedTime.TotalMilliseconds >=
                persistenceConfiguration.BadQueryWarning.GetSlowQueryMillisecondsThreshold(forWriteQuery))
    ...
    public static void LogSlowQueryBadQueryWarning(TimeSpan queryElapsedTime,
        ILogger logger,
        IPlatformPersistenceConfiguration persistenceConfiguration,
        string loggingStackTrace,
        Func<string>? resultQueryStringBuilder)
    {
        logger.Log(persistenceConfiguration.BadQueryWarning.IsLogWarningAsError ? LogLevel.Error : LogLevel.Warning,
    ...
    public static void LogTooMuchDataInMemoryBadQueryWarning(int totalCount,
        ILogger logger,
        IPlatformPersistenceConfiguration persistenceConfiguration,
        string loggingStackTrace,
        Func<string>? resultQueryStringBuilder)
    ...
    public Task<int> CountAsync<TEntity>(Expression<Func<TEntity, bool>>? predicate = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity;

    public Task<int> CountAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    public Task<bool> AnyAsync<TEntity>(Expression<Func<TEntity, bool>>? predicate = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity;

    public Task<bool> AnyAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    Task<TResult> FirstOrDefaultAsync<TEntity, TResult>(Func<IQueryable<TEntity>, IQueryable<TResult>> queryBuilder,
        CancellationToken cancellationToken = default)
        where TEntity : class, IEntity;

    Task<List<T>> GetAllAsync<T>(IQueryable<T> source, CancellationToken cancellationToken = default);

    Task<List<TResult>> GetAllAsync<TEntity, TResult>(Func<IQueryable<TEntity>, IQueryable<TResult>> queryBuilder,
        CancellationToken cancellationToken = default)
        where TEntity : class, IEntity;

    public Task<List<TEntity>> CreateManyAsync<TEntity, TPrimaryKey>(List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<TEntity> UpdateAsync<TEntity, TPrimaryKey>(TEntity entity,
        bool dismissSendEvent,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<List<TEntity>> UpdateManyAsync<TEntity, TPrimaryKey>(List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public async Task<List<TEntity>> UpdateManyAsync<TEntity, TPrimaryKey>(Expression<Func<TEntity, bool>> predicate,
        ...
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        ...
        return await UpdateManyAsync<TEntity, TPrimaryKey>(toUpdateEntities, dismissSendEvent, eventCustomConfig,
            cancellationToken);
    }

    public Task<TEntity> DeleteAsync<TEntity, TPrimaryKey>(TPrimaryKey entityId,
        bool dismissSendEvent,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<TEntity> DeleteAsync<TEntity, TPrimaryKey>(TEntity entity,
        bool dismissSendEvent,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<List<TEntity>> DeleteManyAsync<TEntity, TPrimaryKey>(List<TPrimaryKey> entityIds,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<List<TEntity>> DeleteManyAsync<TEntity, TPrimaryKey>(List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public async Task<List<TEntity>> DeleteManyAsync<TEntity, TPrimaryKey>(Expression<Func<TEntity, bool>> predicate,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        ...
        return await DeleteManyAsync<TEntity, TPrimaryKey>(toDeleteEntities, dismissSendEvent, eventCustomConfig,
            cancellationToken);
    }

    public Task<TEntity> CreateAsync<TEntity, TPrimaryKey>(TEntity entity,
        bool dismissSendEvent,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public Task<TEntity> CreateOrUpdateAsync<TEntity, TPrimaryKey>(TEntity entity,
    ...
    ///     Example for customCheckExistingPredicate: createOrUpdateEntity => existingEntity => existingEntity.XXX ==
    ///     createOrUpdateEntity.XXX
    /// </summary>
    public Task<List<TEntity>> CreateOrUpdateManyAsync<TEntity, TPrimaryKey>(List<TEntity> entities,
        Func<TEntity, Expression<Func<TEntity, bool>>>? customCheckExistingPredicateBuilder = null,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent>? eventCustomConfig = null,
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new();

    public async Task EnsureEntitiesValid<TEntity, TPrimaryKey>(List<TEntity> entities,
