...
public static class PlatformInboxMessageBusConsumerHelper
{
    ...
    {
        if (handleExistingInboxMessage != null && handleExistingInboxMessage.ConsumeStatus !=
            PlatformInboxBusMessage.ConsumeStatuses.Processed)
            await ExecuteDirectlyConsumerWithExistingInboxMessage(handleExistingInboxMessage,
                consumer,
                serviceProvider,
                message,
                routingKey,
            ...
            await SaveAndTryConsumeNewInboxMessageAsync(rootServiceProvider,
                consumer,
        ...
        CancellationToken cancellationToken) where TMessage : class, new()
    {
        var (toProcessInboxMessage, existedInboxMessage) =
            await GetOrCreateToProcessInboxMessage(consumer, inboxBusMessageRepository, message,
                routingKey, cancellationToken);

        if (existedInboxMessage == null ||
            PlatformInboxBusMessage.CanHandleMessagesExpr(inboxConfig.MessageProcessingMaxSeconds).Compile()(
                existedInboxMessage))
        {
            if (handleInUow != null && !handleInUow.IsPseudoTransactionUow())
            {
                handleInUow.OnCompletedActions.Add(async () => await ExecuteConsumerForNewInboxMessage(
                    rootServiceProvider,
                    consumer.GetType(),
                    message,
            ...
            else
            {
                ...
                if (allowProcessInBackgroundThread || toProcessInboxMessage == existedInboxMessage)
                    Util.TaskRunner.QueueActionInBackground(() => ExecuteConsumerForNewInboxMessage(rootServiceProvider,
                            consumer.GetType(),
                            message,
                            toProcessInboxMessage,
                            routingKey,
                    ...
                    await ExecuteConsumerForNewInboxMessage(rootServiceProvider,
                        consumer.GetType(),
                        message,
                        toProcessInboxMessage,
                        routingKey,
    ...
    private static async Task<ValueTuple<PlatformInboxBusMessage, PlatformInboxBusMessage>>
        GetOrCreateToProcessInboxMessage<TMessage>(IPlatformApplicationMessageBusConsumer<TMessage> consumer,
            IPlatformInboxBusMessageRepository inboxBusMessageRepository,
            TMessage message,
            string routingKey,
            CancellationToken cancellationToken) where TMessage : class, new()
    {
        return await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(async () =>
            {
                ...
                var newInboxMessage = existedInboxMessage == null
                    ? await CreateNewInboxMessageAsync(inboxBusMessageRepository,
                        consumer.GetType(),
                        message,
                        routingKey,
        ...
        CancellationToken cancellationToken) where TMessage : class, new()
    {
        try
        {
            ...
            if (autoDeleteProcessedMessage)
                await DeleteExistingInboxProcessedMessageAsync(serviceProvider,
                ...
                try
                {
                    await UpdateExistingInboxProcessedMessageAsync(serviceProvider,
                        existingInboxMessage,
                        cancellationToken);
                }
                catch (Exception)
        ...
        {
            await UpdateExistingInboxFailedMessageAsync(serviceProvider,
                existingInboxMessage,
                message,
                consumer,
                routingKey,
    ...
    public static async Task<PlatformInboxBusMessage> CreateNewInboxMessageAsync<TMessage>(
        ...
        CancellationToken cancellationToken = default) where TMessage : class, new()
    {
        var newInboxMessage = PlatformInboxBusMessage.Create(message,
        ...
        var result = await inboxBusMessageRepository.CreateAsync(newInboxMessage,
            true,
            null,
            cancellationToken);
    ...
    public static async Task UpdateExistingInboxProcessedMessageAsync(IServiceProvider serviceProvider,
        ...
        CancellationToken cancellationToken = default)
    {
        await serviceProvider.ExecuteInjectScopedAsync(async (IPlatformInboxBusMessageRepository inboxBusMessageRepo) =>
        {
            ...
            await inboxBusMessageRepo.UpdateAsync(toUpdateInboxMessage, true, null,
                cancellationToken);
        });
    }

    public static async Task DeleteExistingInboxProcessedMessageAsync(IServiceProvider serviceProvider,
        PlatformInboxBusMessage existingInboxMessage,
        Func<ILogger> loggerFactory,
        CancellationToken cancellationToken = default)
    {
            ...
            await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(
                async () =>
                {
                    await serviceProvider.ExecuteInjectScopedAsync(
                        async (IPlatformInboxBusMessageRepository inboxBusMessageRepo) =>
                        {
                            await inboxBusMessageRepo.DeleteAsync(existingInboxMessage.Id, true, null,
                                cancellationToken);
                        });
                },
    ...
    public static async Task UpdateExistingInboxFailedMessageAsync<TMessage>(IServiceProvider serviceProvider,
        PlatformInboxBusMessage existingInboxMessage,
        TMessage message,
        IPlatformApplicationMessageBusConsumer<TMessage> consumer,
        string routingKey,
                    ...
                    await serviceProvider.ExecuteInjectScopedAsync(
                        async (IPlatformInboxBusMessageRepository inboxBusMessageRepo) =>
                        {
                            ...
                            var latestCurrentExistingInboxMessage =
                                await inboxBusMessageRepo.FirstOrDefaultAsync(p => p.Id == existingInboxMessage.Id);

                            if (latestCurrentExistingInboxMessage != null)
                                await UpdateExistingInboxFailedMessageAsync(exception,
                                    retryProcessFailedMessageInSecondsUnit,
                                    cancellationToken,
    ...
    public static async Task UpdateExistingInboxFailedMessageAsync(IServiceProvider serviceProvider,
        string id,
        Exception exception,
        double retryProcessFailedMessageInSecondsUnit,
        Func<ILogger> loggerFactory,
            ...
            await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(async () =>
                {
                    await serviceProvider.ExecuteInjectScopedAsync(
                        async (IPlatformInboxBusMessageRepository inboxBusMessageRepo) =>
                        {
                            var existingInboxMessage =
                                await inboxBusMessageRepo.FirstOrDefaultAsync(p => p.Id == id, cancellationToken);
    ...
    private static async Task UpdateExistingInboxFailedMessageAsync(Exception exception,
        double retryProcessFailedMessageInSecondsUnit,
        CancellationToken cancellationToken,
        PlatformInboxBusMessage existingInboxMessage,
        IPlatformInboxBusMessageRepository inboxBusMessageRepo)
    {
        ...
        existingInboxMessage.LastConsumeError =
            PlatformInboxBusMessage.SerializeOneLevelExceptionForLastConsumeError(exception);
        ...
        await inboxBusMessageRepo.UpdateAsync(existingInboxMessage, true, null,
            cancellationToken);
    }
}