...
public class PlatformInboxBusMessageCleanerHostedService : PlatformIntervalProcessHostedService
{
    ...
    public PlatformInboxBusMessageCleanerHostedService(IServiceProvider serviceProvider,
    ...
    protected override async Task IntervalProcessAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(() => CleanInboxEventBusMessage(cancellationToken),
    ...
    protected async Task CleanInboxEventBusMessage(CancellationToken cancellationToken)
    {
        var totalProcessedMessages = await ServiceProvider.ExecuteScopedAsync(p =>
            p.ServiceProvider.GetRequiredService<IPlatformInboxBusMessageRepository>()
                .CountAsync(p => p.ConsumeStatus == PlatformInboxBusMessage.ConsumeStatuses.Processed,
                    cancellationToken));
        ...
        CancellationToken cancellationToken)
    {
        await ServiceProvider.ExecuteInjectScopedScrollingPagingAsync<PlatformInboxBusMessage>(
            ...
            {
                var toDeleteMessages = await inboxEventBusMessageRepo.GetAllAsync(query => query
                ...
                if (toDeleteMessages.Count > 0)
                    await inboxEventBusMessageRepo.DeleteManyAsync(toDeleteMessages,
                        true,
                        null,
                        cancellationToken);
        ...
        Logger.LogInformation("CleanInboxEventBusMessage success. Number of deleted messages: {DeletedMessagesCount}",
    ...
    {
        var toDeleteMessageCount = await ServiceProvider.ExecuteScopedAsync(p =>
            p.ServiceProvider.GetRequiredService<IPlatformInboxBusMessageRepository>()
                .CountAsync(
                    PlatformInboxBusMessage.ToCleanExpiredMessagesByTimeExpr(DeleteProcessedMessageInSeconds(),
        ...
        {
            await ServiceProvider.ExecuteInjectScopedScrollingPagingAsync<PlatformInboxBusMessage>(
                toDeleteMessageCount / NumberOfDeleteMessagesBatch(),
                async (IPlatformInboxBusMessageRepository inboxEventBusMessageRepo) =>
                {
                    var expiredMessages = await inboxEventBusMessageRepo.GetAllAsync(query => query
                            .Where(PlatformInboxBusMessage.ToCleanExpiredMessagesByTimeExpr(
                    ...
                    if (expiredMessages.Count > 0)
                        await inboxEventBusMessageRepo.DeleteManyAsync(expiredMessages,
                            true,
                            null,
                            cancellationToken);
            ...
            Logger.LogInformation(
                "CleanInboxEventBusMessage success. Number of deleted messages: {DeletedMessageCount}",
                toDeleteMessageCount);
        }
    }
}