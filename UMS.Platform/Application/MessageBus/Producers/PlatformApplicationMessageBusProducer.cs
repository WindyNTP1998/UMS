...
namespace UMS.Platform.Application.MessageBus.Producers;

public interface IPlatformApplicationBusMessageProducer
{
    /// <summary>
    ///     Send message to bus. Routing key will be
    ///     "{TMessage.MessageGroup}.{ApplicationSettingContext.ApplicationName}.{TMessage.MessageType}.{
    ///     <see cref="messageAction" />}" <br />
    ///     If forceUseDefaultRoutingKey = true or message is not <see cref="IPlatformSelfRoutingKeyBusMessage" /> then Use
    ///     Default RoutingKey <see cref="PlatformBusMessageRoutingKey.BuildDefaultRoutingKey" />
    /// </summary>
    ...
    Task<TMessage> SendAsync<TMessage, TMessagePayload>(string trackId,
        ...
        where TMessage : class, IPlatformWithPayloadBusMessage<TMessagePayload>, IPlatformSelfRoutingKeyBusMessage,
        IPlatformTrackableBusMessage, new()
    ...
    /// </summary>
    Task<TMessage> SendAsync<TMessage>(TMessage message,
        bool autoSaveOutboxMessage = true,
        bool forceUseDefaultRoutingKey = false,
...
{
    public PlatformApplicationBusMessageProducer(IServiceProvider serviceProvider,
    ...
    public async Task<TMessage> SendAsync<TMessage, TMessagePayload>(string trackId,
        TMessagePayload messagePayload,
        string messageGroup = null,
        string messageAction = null,
        bool autoSaveOutboxMessage = true,
        ...
        where TMessagePayload : class, new()
    {
        var message = BuildPlatformBusMessage<TMessage, TMessagePayload>(trackId, messagePayload, messageGroup,
            messageAction);

        return await SendMessageAsync(message,
            forceUseDefaultRoutingKey
                ? PlatformBusMessageRoutingKey.BuildDefaultRoutingKey(message.GetType(),
    ...
    public async Task<TMessage> SendAsync<TMessage>(TMessage message,
        bool autoSaveOutboxMessage = true,
        bool forceUseDefaultRoutingKey = false,
        string sourceOutboxUowId = null,
    ...
    {
        return await SendMessageAsync(message,
            forceUseDefaultRoutingKey || message.As<IPlatformSelfRoutingKeyBusMessage>() == null
                ? PlatformBusMessageRoutingKey.BuildDefaultRoutingKey(message.GetType(),
                    ApplicationSettingContext.ApplicationName)
    ...
    protected virtual async Task<TMessage> SendMessageAsync<TMessage>(TMessage message,
        string routingKey,
        ...
        where TMessage : class, new()
    {
        ...
        if (autoSaveOutboxMessage && HasOutboxMessageSupport())
        {
            var outboxEventBusProducerHelper =
                ServiceProvider.GetRequiredService<PlatformOutboxMessageBusProducerHelper>();
    ...
    protected TMessage BuildPlatformBusMessage<TMessage, TMessagePayload>(string trackId,
        TMessagePayload messagePayload, string messageGroup, string messageAction)
        where TMessage : class, IPlatformWithPayloadBusMessage<TMessagePayload>, IPlatformSelfRoutingKeyBusMessage,
        IPlatformTrackableBusMessage, new()
        where TMessagePayload : class, new()
    {
        return PlatformBusMessage<TMessagePayload>.New<TMessage>(trackId,
            messagePayload,
    ...
    {
        public Task<TMessage> SendAsync<TMessage>(TMessage message,
            string routingKey,
            CancellationToken cancellationToken = default) where TMessage : class, new()
        {
