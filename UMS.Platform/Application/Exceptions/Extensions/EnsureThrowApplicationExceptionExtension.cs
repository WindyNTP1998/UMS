...
public static class EnsureThrowApplicationExceptionExtension
{
    ...
    {
        return applicationVal.IsValid
            ? applicationVal.Value
            : throw new PlatformApplicationException(applicationVal.ErrorsMsg());
    }
    ...
    public static async Task<T> EnsureApplicationLogicValidAsync<T>(this Task<T> valueTask, Func<T, bool> must,
        string errorMsg)
    {
    ...
    public static async Task<T> EnsureApplicationLogicValidAsync<T>(
        this Task<PlatformValidationResult<T>> applicationValTask)
    {
        var applicationVal = await applicationValTask;
        return applicationVal.EnsureApplicationLogicValid();
    }

    public static async Task<object> EnsureApplicationLogicValidAsync(
        this Task<PlatformValidationResult> applicationValTask)
    {
        var applicationVal = await applicationValTask;
        return applicationVal.EnsureApplicationLogicValid();
    }
