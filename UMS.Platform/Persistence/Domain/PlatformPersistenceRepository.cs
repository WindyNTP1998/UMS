...
public abstract class PlatformPersistenceRepository<TEntity, TPrimaryKey, TUow, TDbContext>
    : PlatformRepository<TEntity, TPrimaryKey, TUow>
    where TEntity : class, IEntity<TPrimaryKey>, new()
    where TUow : class, IPlatformPersistenceUnitOfWork<TDbContext>
    where TDbContext : IPlatformDbContext
{
    protected PlatformPersistenceRepository(IUnitOfWorkManager unitOfWorkManager,
        IPlatformCqrs cqrs,
        IServiceProvider serviceProvider) : base(unitOfWorkManager, cqrs, serviceProvider)
    {
        ...
        Logger = serviceProvider.GetRequiredService<ILoggerFactory>()
            .CreateLogger(typeof(PlatformPersistenceRepository<,,,>));
    }

    /// <summary>
    ///     Return CurrentActiveUow db context if exist or. <br />
    ///     Auto use GlobalUow if there's no current active uow. <br />
    ///     Support for old system code or other application want to use platform repository inherit DbContext but without open
    ///     new uow
    /// </summary>
        ...
        Func<IUnitOfWork, Task<TResult>> action)
    {
        return await ExecuteWithBadQueryWarningForWriteHandling(
            _ => base.ExecuteAutoOpenUowUsingOnceTimeForWrite(action), null);
    }

    protected async Task<TResult> ExecuteWithBadQueryWarningForWriteHandling<TResult>(
        Func<IUnitOfWork, Task<TResult>> action, IUnitOfWork uow)
    {
        if (PersistenceConfiguration.BadQueryWarning.IsEnabled)
            return await IPlatformDbContext.ExecuteWithBadQueryWarningHandling<TResult, TEntity>(() => action(uow),
                Logger,
    ...
    public abstract Task<List<TSource>> ToListAsync<TSource>(IEnumerable<TSource> source,
    ...
    ///     Default as Enumerable from IQueryable still like Queryable which cause error query could not be translated for free
    ///     select using constructor map for example
    /// </summary>
    public abstract IAsyncEnumerable<TSource> ToAsyncEnumerable<TSource>(IEnumerable<TSource> source,
        CancellationToken cancellationToken = default);

    public abstract Task<TSource> FirstOrDefaultAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    public abstract Task<TSource> FirstAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    public abstract Task<int> CountAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    public abstract Task<bool> AnyAsync<TSource>(IQueryable<TSource> source,
        CancellationToken cancellationToken = default);

    public override Task<TEntity> GetByIdAsync(TPrimaryKey id,
        CancellationToken cancellationToken = default,
        params Expression<Func<TEntity, object>>[] loadRelatedEntities)
    {
        return ExecuteAutoOpenUowUsingOnceTimeForRead((_, query) =>
                FirstOrDefaultAsync(query.Where(p => p.Id!.Equals(id)), cancellationToken)
                    .EnsureFound($"{typeof(TEntity).Name} with Id {id} is not found"),
            loadRelatedEntities);
    }

    public override Task<List<TEntity>> GetByIdsAsync(List<TPrimaryKey> ids,
        CancellationToken cancellationToken = default,
        params Expression<Func<TEntity, object>>[] loadRelatedEntities)
    {
    ...
    public override Task<List<TEntity>> GetAllAsync(IQueryable<TEntity> query,
        CancellationToken cancellationToken = default)
    {
    ...
    public override IAsyncEnumerable<TEntity> GetAllAsyncEnumerable(IQueryable<TEntity> query,
        CancellationToken cancellationToken = default)
    {
    ...
    public override Task<TEntity> FirstAsync(Expression<Func<TEntity, bool>> predicate = null,
        CancellationToken cancellationToken = default,
        params Expression<Func<TEntity, object>>[] loadRelatedEntities)
    {
        return ExecuteAutoOpenUowUsingOnceTimeForRead((_, query) =>
                FirstOrDefaultAsync(query.WhereIf(predicate != null, predicate), cancellationToken)
    ...
    public override Task<int> CountAsync<TQueryItemResult>(
        ...
        CancellationToken cancellationToken = default)
    {
        return ExecuteAutoOpenUowUsingOnceTimeForRead((_, query) => CountAsync(queryBuilder(query), cancellationToken),
            Array.Empty<Expression<Func<TEntity, object>>>());
    }
    ...
    public override Task<List<TSelector>> GetAllAsync<TSelector>(
        ...
        params Expression<Func<TEntity, object>>[] loadRelatedEntities)
    {
        return ExecuteAutoOpenUowUsingOnceTimeForRead((_, query) => ToListAsync(queryBuilder(query), cancellationToken),
            loadRelatedEntities);
    }
        ...
        if (IsDistributedTracingEnabled)
            using (var activity = IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateAsync)}"))
            {
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                    .CreateAsync<TEntity, TPrimaryKey>(entity, dismissSendEvent, eventCustomConfig,
                        cancellationToken));
            }
    ...
    public override async Task<TEntity> CreateOrUpdateAsync(TEntity entity,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateOrUpdateAsync)}"))
            {
                activity?.AddTag("EntityType", typeof(TEntity).FullName);
                activity?.AddTag("Entity", entity.ToJson());
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                    .CreateOrUpdateAsync<TEntity, TPrimaryKey>(entity, null, dismissSendEvent,
        ...
        if (IsDistributedTracingEnabled)
                   ...
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateOrUpdateAsync)}"))
            {
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow => GetUowDbContext(uow)
                    .CreateOrUpdateAsync<TEntity, TPrimaryKey>(entity, customCheckExistingPredicate, dismissSendEvent,
                        eventCustomConfig, cancellationToken));
            }
    ...
    public override async Task<List<TEntity>> CreateOrUpdateManyAsync(List<TEntity> entities,
        bool dismissSendEvent = false,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateOrUpdateManyAsync)}"))
            {
                activity?.AddTag("EntityType", typeof(TEntity).FullName);
                activity?.AddTag("Entity", entities.ToJson());

                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow => GetUowDbContext(uow)
                    .CreateOrUpdateManyAsync<TEntity, TPrimaryKey>(entities,
    ...
    public override async Task<List<TEntity>> CreateOrUpdateManyAsync(IUnitOfWork uow,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
                   ...
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateOrUpdateManyAsync)}"))
            {
                ...
                return await ExecuteWithBadQueryWarningForWriteHandling(uow => GetUowDbContext(uow)
                        .CreateOrUpdateManyAsync<TEntity, TPrimaryKey>(entities,
                            customCheckExistingPredicateBuilder,
                            dismissSendEvent,
                            eventCustomConfig,
        ...
        CancellationToken cancellationToken = default)
    {
        if (IsDistributedTracingEnabled)
            using (var activity = IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(UpdateAsync)}"))
            {
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                    .UpdateAsync<TEntity, TPrimaryKey>(entity, dismissSendEvent, eventCustomConfig,
                        cancellationToken));
            }
    ...
    public override async Task<TEntity> DeleteAsync(TPrimaryKey entityId,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
        if (IsDistributedTracingEnabled)
            using (var activity = IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(DeleteAsync)}"))
            {
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                        .DeleteAsync<TEntity, TPrimaryKey>(entityId, dismissSendEvent, eventCustomConfig,
                            cancellationToken));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        if (IsDistributedTracingEnabled)
            using (var activity = IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(DeleteAsync)}"))
            {
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                    .DeleteAsync<TEntity, TPrimaryKey>(entity, dismissSendEvent, eventCustomConfig,
                        cancellationToken));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(CreateManyAsync)}"))
            {
                activity?.AddTag("EntityType", typeof(TEntity).FullName);
                activity?.AddTag("Entity", entities.ToJson());
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                        .CreateManyAsync<TEntity, TPrimaryKey>(entities, dismissSendEvent, eventCustomConfig,
                            cancellationToken));
            }
    ...
    public override async Task<List<TEntity>> CreateManyAsync(IUnitOfWork uow,
        List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    ...
    public override async Task<List<TEntity>> UpdateManyAsync(List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(UpdateManyAsync)}"))
            {
                activity?.AddTag("EntityType", typeof(TEntity).FullName);
                activity?.AddTag("Entity", entities.ToJson());
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                        .UpdateManyAsync<TEntity, TPrimaryKey>(entities, dismissSendEvent, eventCustomConfig,
                            cancellationToken));
            }
    ...
    public override async Task<List<TEntity>> UpdateManyAsync(IUnitOfWork uow,
        List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    ...
    public override async Task<List<TEntity>> DeleteManyAsync(List<TPrimaryKey> entityIds,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(DeleteManyAsync)}"))
            {
                activity?.AddTag("EntityType", typeof(TEntity).FullName);
                ...
                return await ExecuteAutoOpenUowUsingOnceTimeForWrite(uow =>
                    GetUowDbContext(uow)
                        .DeleteManyAsync<TEntity, TPrimaryKey>(entityIds, dismissSendEvent, eventCustomConfig,
                            cancellationToken));
            }
    ...
    public override async Task<List<TEntity>> DeleteManyAsync(IUnitOfWork uow,
        List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
        ...
        if (IsDistributedTracingEnabled)
                   ...
                   IPlatformRepository.ActivitySource.StartActivity($"Repository.{nameof(DeleteManyAsync)}"))
            {
                ...
                return await ExecuteWithBadQueryWarningForWriteHandling(
                    uow => GetUowDbContext(uow)
                        .DeleteManyAsync<TEntity, TPrimaryKey>(entities, dismissSendEvent, eventCustomConfig,
                            cancellationToken),
                    uow);
            }
    ...
    public override async Task<List<TEntity>> DeleteManyAsync(List<TEntity> entities,
        bool dismissSendEvent = false,
        Action<PlatformCqrsEntityEvent> eventCustomConfig = null,
        CancellationToken cancellationToken = default)
    {
...
public abstract class PlatformPersistenceRootRepository<TEntity, TPrimaryKey, TUow, TDbContext>
    ...
    where TDbContext : IPlatformDbContext
{
    protected PlatformPersistenceRootRepository(IUnitOfWorkManager unitOfWorkManager, IPlatformCqrs cqrs,
        IServiceProvider serviceProvider) : base(unitOfWorkManager,
        cqrs,
        serviceProvider)
    {
    }
