...
using UMS.Platform.Persistence.Domain;
using UMS.Platform.Persistence.Services;

namespace UMS.Platform.Persistence;
...
public interface IPlatformPersistenceModule : IPlatformModule
{
    ...
    public static async Task ExecuteDependencyPersistenceModuleMigrateApplicationData(List<Type> moduleTypeDependencies,
        IServiceProvider serviceProvider)
    {
        await moduleTypeDependencies
            ...
            .Select(moduleType => new
            {
                ...
                serviceProvider.GetService(moduleType).As<IPlatformPersistenceModule>().ExecuteInitPriority
            })
            ...
            .ForEachAsync(async moduleType =>
            {
                await serviceProvider.ExecuteScopedAsync(scope =>
                    scope.ServiceProvider.GetService(moduleType)
                        .As<IPlatformPersistenceModule>()
                        .MigrateApplicationDataAsync(scope));
            });
    }
}
...
public abstract class PlatformPersistenceModule : PlatformModule, IPlatformPersistenceModule
{
    public new const int DefaultExecuteInitPriority =
        PlatformModule.DefaultExecuteInitPriority + ExecuteInitPriorityNextLevelDistance * 2;

    protected PlatformPersistenceModule(IServiceProvider serviceProvider, IConfiguration configuration) : base(
    ...
    {
        return Util.ListBuilder.NewArray(IPlatformRepository.ActivitySource.Name,
    ...
    {
        serviceCollection.Register<IUnitOfWorkManager, PlatformDefaultPersistenceUnitOfWorkManager>(ServiceLifeTime
            .Scoped);

        serviceCollection.RegisterAllFromType(typeof(IUnitOfWorkManager),
            Assembly,
    ...
    private void RegisterRepositories(IServiceCollection serviceCollection)
    {
        ...
        if (RegisterLimitedRepositoryImplementationTypes()?.Any() == true)
            RegisterLimitedRepositoryImplementationTypes()
                .ForEach(repositoryImplementationType =>
                    serviceCollection.RegisterAllForImplementation(repositoryImplementationType));
        else
    ...
    where TDbContext : class, IPlatformDbContext<TDbContext>
{
    ...
    public override async Task MigrateApplicationDataAsync(IServiceScope serviceScope)
    {
        ...
        await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(
            async () =>
            {
                await serviceScope.ServiceProvider.GetRequiredService<TDbContext>()
                    .MigrateApplicationDataAsync(serviceScope.ServiceProvider);
            },
    ...
    public override async Task InitializeDb(IServiceScope serviceScope)
    {
        ...
        await Util.TaskRunner.WaitRetryThrowFinalExceptionAsync(
            ...
            {
                await serviceScope.ServiceProvider.GetRequiredService<TDbContext>()
                    .Initialize(serviceScope.ServiceProvider);
            },
            retryAttempt => 10.Seconds(),
            DefaultDbInitAndMigrationRetryCount,
            exception => Logger.LogError(exception,
    ...
    {
        serviceCollection.Register(sp => new PlatformPersistenceConfiguration<TDbContext>()
