...
namespace UMS.Platform.Persistence.Services;
...
public interface IPlatformFullTextSearchPersistenceService : IPersistenceService
{
    ...
    ///     Ex: Input searchText: "abc def", inFullTextSearchProps: [p => p.PropA, p => p.PropB] will return query which is
    ///     (p.PropA contains ("abc" AND "def") OR p.PropB contains ("abc" AND "def")).
    /// </summary>
...
public abstract class PlatformFullTextSearchPersistenceService : IPlatformFullTextSearchPersistenceService
{
        ...
        Expression<Func<T, object>>[] includeStartWithProps = null) where T : class
    {
        var byFirstSupportQueryHelperFilterQuery = !IsSupportQuery(query)
            ? TrySearchByFirstSupportQueryHelper(query, searchText, inFullTextSearchProps,
                fullTextAccurateMatch, includeStartWithProps)
            : null;

        return byFirstSupportQueryHelperFilterQuery ?? DoSearch(query, searchText, inFullTextSearchProps,
            fullTextAccurateMatch, includeStartWithProps);
    }

    public abstract bool IsSupportQuery<T>(IQueryable<T> query) where T : class;

    protected abstract IQueryable<T> DoSearch<T>(IQueryable<T> query,
        string searchText,
        Expression<Func<T, object>>[] inFullTextSearchProps,
        bool fullTextAccurateMatch = true,
        Expression<Func<T, object>>[] includeStartWithProps = null) where T : class;

    protected IQueryable<T> TrySearchByFirstSupportQueryHelper<T>(IQueryable<T> query,
        string searchText,
        Expression<Func<T, object>>[] inFullTextSearchProps,
        bool exactMatch,
        Expression<Func<T, object>>[] includeStartWithProps = null) where T : class
    {
        ...
        return otherSupportHelper?.Search(query,
            searchText,
