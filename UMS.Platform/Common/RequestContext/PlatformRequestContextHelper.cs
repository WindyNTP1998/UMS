...
namespace UMS.Platform.Common.RequestContext;
...
public static class PlatformRequestContextHelper
{
    public static bool TryGetValue<T>(IDictionary<string, object> requestContext, string contextKey, out T item)
    {
        ...
        if (originalValue != null)
        {
            ...
            {
                var isParsedSuccess =
                    TryGetParsedValuesFromStringValues(out item, Util.ListBuilder.New(originalValueStr));
    ...
    public static bool TryGetParsedValuesFromStringValues<T>(out T foundValue, List<string> stringValues)
    {
        ...
        if (isTryGetListValueSuccess) return true;

        return PlatformJsonSerializer.TryDeserialize(stringValues.LastOrDefault(),
            out foundValue);
    }
    ...
    public static bool TryGetParsedListValueFromUserClaimStringValues<T>(List<string> matchedClaimStringValues,
        out T foundValue)
    {
        ...
        if (firstValueListInterface != null)
        {
            ...
            var parsedItemList = matchedClaimStringValues
                .Select(matchedClaimStringValue =>
                {
                    if (listItemType == typeof(string))
                        return new
                        {
                            itemDeserializedValue = (object)matchedClaimStringValue, isParsedItemSucceeded = true
                        };

                    var isParsedItemSucceeded = PlatformJsonSerializer.TryDeserialize(matchedClaimStringValue,
                        listItemType,
            ...
            if (parsedItemList.All(p => p.isParsedItemSucceeded))
            {
                ...
                foundValue =
                    PlatformJsonSerializer.Deserialize<T>(parsedItemList.Select(p => p.itemDeserializedValue).ToJson());
                return true;
            }
        }
