...
///     Example: Utils.String should produce string as output.Utils.Enums should produce enum as output.Utils.Copy should
///     only do the copy data functional.
/// </summary>
public static partial class Util
{
    public static class FullTextSearchChecker
    {
        public static bool IsFullTextSearchMatch(string targetText,
            string searchText,
            bool exactMatchAllWords = false)
        {
            ...
            var isMatchWords = exactMatchAllWords
                ? searchWords.All(word => Regex.IsMatch(noDiacriticsTargetText,
                    GetMatchWordRegexPattern(word),
                    RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
                : searchWords.Any(word => Regex.IsMatch(noDiacriticsTargetText,
                    GetMatchWordRegexPattern(word),
                    RegexOptions.IgnoreCase | RegexOptions.CultureInvariant));

            return Regex.IsMatch(noDiacriticsTargetText,
