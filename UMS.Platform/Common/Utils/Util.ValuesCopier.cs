...
public static partial class Util
{
    public static class ValuesCopier
    {
        ...
        public static void CopyValues<TSource, TTarget>(TSource source, TTarget target,
            params Expression<Func<TSource, object>>[] ignoreProperties)
            where TSource : class
            where TTarget : class
        {
        ...
        internal static class PropertyCopier<TSource, TTarget>
        {
                ...
                params Expression<Func<TSource, object>>[] ignoreProperties)
            {
                ...
                var filteredSourceProperties =
                    SourceProperties.Where(p => !ignorePropertiesDic.ContainsKey(p.Name)).ToList();
                var filteredTargetProperties =
                    TargetProperties.Where(p => !ignorePropertiesDic.ContainsKey(p.Name)).ToList();

                for (var i = 0; i < filteredSourceProperties.Count; i++)
                    filteredTargetProperties[i]
                        .SetValue(target, filteredSourceProperties[i].GetValue(source, null), null);
            }
