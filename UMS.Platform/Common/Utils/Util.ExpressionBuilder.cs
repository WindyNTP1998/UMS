...
public static partial class Util
{
    ...
    {
        /// <summary>
        ///     Return a contains <see cref="containStringValue" /> expression on string property by <see cref="propertyName" /> of
        ///     instance type of <see cref="T" />
        ///     Example: BuildStringContainsExpression("Search", "abc") equal to p => p.Search.Contains("abc")
        /// </summary>
        public static Expression<Func<T, bool>> BuildStringContainsExpression<T>(string propertyName,
            string containStringValue)
        {
            ...
            var containsMethodCallExpr =
                BuildMethodCallExpr(propertyExpr,
                    typeof(string),
                    "Contains",
                    new object[]
                    {
            ...
            return Expression.Lambda<Func<T, bool>>(containsMethodCallExpr, parameterExpr);
        }

        /// <summary>
        ///     Return a chain method called expression on a property by <see cref="propertyName" /> of instance type of
        ///     <see cref="TChainTarget" />
        ///     Example: BuildChainExpression("Search", new (string, object[])[] { ("ToLower", null), ("Contains", new[] { "abc" })
        ///     } ) equal to p => p.Search.ToLower().Contains("abc")
        /// </summary>
        ...
        public static Expression<Func<TChainTarget, bool>> BuildChainExpression<TChainTarget>(string propertyName,
            params ValueTuple<string, object[]>[] chainMethods)
        {
            if (!chainMethods.Any()) throw new Exception("At least one chain method must be given");
            ...
            var firstChainMethodCallExpr = BuildMethodCallExpr(chainTargetPropertyExpr,
            ...
            var aggregatedChainMethodCallExpr = chainMethods.Skip(1)
                .Aggregate(firstChainMethodCallExpr,
                    (prevChainMethodCallExpr, chainMethodInfo) => BuildMethodCallExpr(prevChainMethodCallExpr,
                        prevChainMethodCallExpr.Method.ReturnType,
                        chainMethodInfo.Item1,
                        chainMethodInfo.Item2));

            return Expression.Lambda<Func<TChainTarget, bool>>(aggregatedChainMethodCallExpr,
                chainTargetParameterExpr);
        }

        private static MethodCallExpression BuildMethodCallExpr(Expression calledOnExpressionTarget,
            ...
            object[] methodParams)
        {
            ...
            return Expression.Call(calledOnExpressionTarget,
