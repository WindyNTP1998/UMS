...
namespace UMS.Platform.Common.Logging.BackgroundThreadFullStackTrace;

/// <summary>
///     Used so store stack-trace for logging BEFORE Task.Run to run some new action in background. BECAUSE after call
///     function in new background thread run, the stack trace get lost, only trace the last position to Task.Run
/// </summary>
public interface IPlatformBackgroundThreadFullStackTraceContextAccessor
{
    [AllowNull] string Current { get; set; }
}
...
public class PlatformBackgroundThreadFullStackTraceContextAccessor
    : IPlatformBackgroundThreadFullStackTraceContextAccessor
{
