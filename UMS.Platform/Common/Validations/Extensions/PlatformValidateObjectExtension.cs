...
public static class PlatformValidateObjectExtension
{
    ...
    public static PlatformValidationResult<TValue> Validate<TValue>(this TValue value,
    ...
    public static PlatformValidationResult<TValue> Validate<TValue>(this TValue value,
        ...
        string? actual)
    {
        return PlatformValidationResult<TValue>.Validate(value,
            () => must(value),
            $"Expected: {expected}".PipeIf(_ => actual.IsNotNullOrEmpty(),
                _ => _ + $".{Environment.NewLine}Actual: {actual}"));
    }
    ...
    public static PlatformValidationResult<TValue> ValidateNot<TValue>(this TValue value,
    ...
    public static PlatformValidationResult<TValue> ValidateNot<TValue>(this TValue value,
        ...
        string? actual)
    {
        return PlatformValidationResult<TValue>.ValidateNot(value,
            () => mustNot(value),
            $"Expected: {expected}".PipeIf(_ => actual.IsNotNullOrEmpty(),
                _ => _ + $".{Environment.NewLine}Actual: {actual}"));
    }
    ...
    public static PlatformValidationResult<T[]> ValidateFound<T>(this T[]? objects,
    ...
    public static PlatformValidationResult<List<T>> ValidateFound<T>(this List<T>? objects,
        string errorMsg = DefaultNotFoundMessage)
    {
        var objectsList = objects?.ToList();
    ...
    public static PlatformValidationResult<IEnumerable<T>> ValidateFound<T>(this IEnumerable<T>? objects,
        string errorMsg = DefaultNotFoundMessage)
    {
        var objectsList = objects?.ToList();
    ...
    public static PlatformValidationResult<List<T>?> ValidateFoundAll<T>(this List<T>? objects,
        ...
        Func<List<T>, string> notFoundObjectsToErrorMsg)
    {
        ...
        return notFoundObjects.Any()
            ? PlatformValidationResult.Invalid(objects, notFoundObjectsToErrorMsg(notFoundObjects))
            : PlatformValidationResult.Valid(objects);
    }

    public static PlatformValidationResult<List<T>> ValidateFoundAllBy<T, TFoundBy>(this List<T> objects,
        ...
        Func<List<TFoundBy>, string>? notFoundByObjectsToErrorMsg = null)
    {
        ...
        return notFoundByObjects.Any()
            ? PlatformValidationResult.Invalid(objects,
                notFoundByObjectsToErrorMsg?.Invoke(notFoundByObjects) ?? DefaultNotFoundMessage)
            : PlatformValidationResult.Valid(objects);
    }

    public static PlatformValidationResult<T> ValidateFound<T>(this T? obj, Func<T, bool> and,
        string errorMsg = DefaultNotFoundMessage)
    {
        return obj != null && and(obj)
            ? PlatformValidationResult.Valid(obj)
            : PlatformValidationResult.Invalid(obj!, errorMsg);
    }

    public static PlatformValidationResult<T> ValidateFound<T>(this T? obj, Func<T, Task<bool>> and,
        string errorMsg = DefaultNotFoundMessage)
    {
        return obj.ValidateFound(p => and(p).GetResult(), errorMsg);
    }

    public static async Task<PlatformValidationResult<T>> ValidateFoundAsync<T>(this T? obj, Func<T, Task<bool>> and,
        string errorMsg = DefaultNotFoundMessage)
    {
    ...
    public static Task<PlatformValidationResult<List<T>>> ValidateFoundAsync<T>(this Task<List<T>>? listTask,
        string errorMsg = DefaultNotFoundMessage)
    {
        return listTask.Then(items => items.ValidateFound(errorMsg));
    }

    public static PlatformValidationResult<IQueryable<T>> ValidateFoundAny<T>(this IQueryable<T> query,
        Expression<Func<T, bool>> any,
        string errorMsg = DefaultNotFoundMessage)
    {
        return query.Any(any)
            ? PlatformValidationResult.Valid(query)
            : PlatformValidationResult.Invalid(query, errorMsg);
    }
