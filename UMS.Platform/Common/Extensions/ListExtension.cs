...
public static class ListExtension
{
    ...
    {
        return items.Concat(new List<T>
        {
            item
        });
    }
    ...
    public static async Task<List<TResult>> ParallelAsync<T, TResult>(this IEnumerable<T> items,
    ...
    public static async Task ForEachAsync<T, TActionResult>(this IEnumerable<T> items,
        Func<T, int, Task<TActionResult>> action)
    {
        var itemsList = items.As<IList<T>>() ?? items.ToList();
    ...
    /// <inheritdoc cref="ForEachAsync{T}(IEnumerable{T},Func{T,int,Task})" />
    public static async Task ForEachAsync<T, TActionResult>(this IList<T> items,
        Func<T, int, Task<TActionResult>> action)
    {
        for (var i = 0; i < items.Count; i++) await action(items[i], i);
    }
    ...
    public static Task<List<TResult>> ParallelAsync<T, TResult>(this IEnumerable<T> items,
    ...
    /// </summary>
    public static async Task<List<TActionResult>> SelectAsync<T, TActionResult>(this IEnumerable<T> items,
    ...
    public static Task<List<TActionResult>> SelectAsync<T, TActionResult>(this IEnumerable<T> items,
    ...
    ///     Example: var listB = await list.SelectAsync((item, itemIndex) => get B async)
    /// </summary>
    public static async Task<List<TActionResult>> SelectAsync<T, TActionResult>(this IList<T> items,
        Func<T, int, Task<TActionResult>> actionAsync)
    {
        var result = new List<TActionResult>();
    ...
    /// <inheritdoc cref="SelectAsync{T,TResult}(IEnumerable{T},Func{T,int,Task{TResult}})" />
    public static Task<List<TActionResult>> SelectAsync<T, TActionResult>(this IList<T> items,
        Func<T, Task<TActionResult>> actionAsync)
    {
        return items.SelectAsync((item, index) => actionAsync(item));
    }
    ...
    /// </summary>
    public static Task<List<TActionResult>> ThenSelect<T, TActionResult>(this Task<IEnumerable<T>> itemsTask,
    ...
    /// <inheritdoc cref="ThenSelect{T,TResult}(Task{IEnumerable{T}},Func{T,int,Task{TResult}})" />
    public static Task<List<TActionResult>> ThenSelect<T, TActionResult>(this Task<List<T>> itemsTask,
        Func<T, int, TActionResult> selector)
    {
        return itemsTask.Then(items => items.Select(selector).ToList());
    }

    public static Task<List<TActionResult>> ThenSelectAsync<T, TActionResult>(this Task<List<T>> itemsTask,
    ...
    public static ValueTuple<Dictionary<TKey, T>, List<TKey>> ToDictionaryWithKeysList<T, TKey>(
        this IEnumerable<T> items, Func<T, TKey> selectKey) where TKey : notnull
    {
    ...
    public static HashSet<TResult> SelectHashset<TSource, TResult>(this IEnumerable<TSource> source,
        Func<TSource, TResult> selector)
    {
        return source.Select(selector).ToHashSet();
    }

    public static IEnumerable<TSource> ConcatIf<TSource>(this IEnumerable<TSource> source, bool @if,
    ...
    public static IEnumerable<TSource> ConcatIf<TSource>(this IEnumerable<TSource> source,
    ...
    public static IEnumerable<TSource> ConcatIf<TSource>(this IEnumerable<TSource> source,
        Func<IEnumerable<TSource>, bool> @if, params TSource[] second)
    {
    ...
    // Add this to fix ambiguous evocation with other library by help compiler to select exact type extension
    public static bool IsNullOrEmpty<TKey, TValue>([NotNullWhen(false)] this Dictionary<TKey, TValue>? items)
        where TKey : notnull
    {
        return items == null || !items.Any();
    }
    ...
    public static IEnumerable<TResult> SelectManyNullable<TSource, TResult>(this IEnumerable<TSource> source,
    ...
    /// </summary>
    public static ValueTuple<List<TSource>, List<TSource>> FilterSplitResult<TSource>(this IEnumerable<TSource> source,
        Func<TSource, bool> predicate)
    {
        ...
        {
            if (predicate(item))
                matchedFilterResult.Add(item);
            else
                remainingFilterResult.Add(item);
        });
