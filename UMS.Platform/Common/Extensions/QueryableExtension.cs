...
public static class QueryableExtension
{
    ...
    public static IOrderedQueryable<T> OrderBy<T>(this IQueryable<T> query, Expression<Func<T, object>> keySelector,
...
internal sealed class ParameterRebinder : ExpressionVisitor
{
    ...
    {
        this.targetToSourceParamsMap =
            targetToSourceParamsMap ?? new Dictionary<ParameterExpression, ParameterExpression>();
    }
    ...
    {
        if (targetToSourceParamsMap.TryGetValue(node, out var replacement)) node = replacement;
