...
public static class TypeExtension
{
    ...
    ///     <br />
    ///     This function used to fix when a Type is generic, get Interfaces will lead to missing fullName => lead to register
    ///     into
    ...
    {
        if (type.FullName != null) return type;
    ...
    public static bool MatchGenericArguments(this Type sourceType, Type targetType)
    {
        return targetType.IsGenericType &&
               sourceType.GetGenericArguments()
                   .ItemsMatch(targetType.GetGenericArguments(),
    ...
    public static Type FindMatchedGenericType(this Type givenType, Type genericType)
    {
        while (true)
        {
            ...
            foreach (var givenInterfaceType in givenInterfaceTypes)
                if (givenInterfaceType.IsGenericType && givenInterfaceType.GetGenericTypeDefinition() ==
                    genericType.GetGenericTypeDefinition())
                    return givenInterfaceType;

            if (givenType.IsGenericType &&
                givenType.GetGenericTypeDefinition() == genericType.GetGenericTypeDefinition())
                return givenType;
