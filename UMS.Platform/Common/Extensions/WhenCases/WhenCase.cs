namespace UMS.Platform.Common.Extensions.WhenCases;
...
public class WhenCase<TSource, TTarget> : WhenCase
{
    ...
    public static WhenCase<TSource, TTarget> WhenValue(TSource source,
        TSource @case,
    ...
    /// </summary>
    public static WhenCase<TSource, TTarget> WhenIs<TSourceIs>(TSource source,
    ...
    public class CaseItem
    {
        ...
        public CaseItem(Func<TSource, bool> @case, Func<TSource, TTarget> then) : this(@case,
            _ => Task.FromResult(then(_)))
        {
        }
        ...
        public CaseItem(Func<TSource, Task<bool>> @case, Func<TSource, TTarget> then) : this(@case,
            _ => Task.FromResult(then(_)))
        {
        }
        ...
        public static implicit operator CaseItem(
            ValueTuple<Func<TSource, bool>, Func<TSource, Task<TTarget>>> caseItemInfo)
        {
...
public class WhenCase<TSource> : WhenCase<TSource, object>
{
    ...
    public WhenCase(TSource source, Func<TSource, bool> @case, Action<TSource> then) : base(source, @case,
        then.ToFunc())
    {
    }
    ...
    public static WhenCase<TSource> WhenValue(TSource source,
        TSource @case,
        Action<TSource> then)
    {
    ...
    public static WhenCase<TSource> WhenIs<TSourceIs>(TSource source,
    ...
    public WhenCase(TSource source, Func<TSource, Task<bool>> @case, Action<TSource> then) : base(source, @case,
        then.ToFunc())
    {
    }
    ...
    public WhenCase(TSource source, Func<TSource, Task<bool>> @case, Action then) : base(source, @case,
        _ => then.ToFunc()())
    {
    }

    public WhenCase(TSource source, Func<TSource, Task<bool>> @case, Func<TSource, Task> then) : base(source, @case,
        then.ToAsyncFunc())
    {
    }
    ...
    public WhenCase(TSource source, Task<bool> @case, Func<TSource, Task> then) : base(source, @case,
        then.ToAsyncFunc())
    {
    }
