...
public static class WithCommonExceptionValidationExtension
{
    ...
    public static async Task<PlatformValidationResult<T>> WithPermissionExceptionAsync<T>(
        this Task<PlatformValidationResult<T>> valTask)
    {
    ...
    public static PlatformValidationResult<T> WithNotFoundException<T>(this PlatformValidationResult<T> val)
    {
        ...
        return val.WithInvalidException(val => new PlatformNotFoundException(val.ErrorsMsg(), objectType));
    }

    public static async Task<PlatformValidationResult<T>> WithNotFoundExceptionAsync<T>(
        this Task<PlatformValidationResult<T>> valTask)
    {
        var applicationVal = await valTask;
