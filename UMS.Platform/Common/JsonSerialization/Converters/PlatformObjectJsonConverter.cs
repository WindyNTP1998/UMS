...
namespace UMS.Platform.Common.JsonSerialization.Converters;
...
public class PlatformObjectJsonConverter : JsonConverter<object>
{
    ...
    {
        if (dynamicObject is JsonElement jsonElement) return TryGetReflectionDynamic(jsonElement);
    ...
    public static dynamic TryGetReflectionDynamic(JsonElement dynamicObjectAsJsonElement)
    {
        switch (dynamicObjectAsJsonElement.ValueKind)
        {
            ...
            {
                if (dynamicObjectAsJsonElement.TryGetInt32(out var valInt32)) return valInt32;
                if (dynamicObjectAsJsonElement.TryGetInt64(out var valInt64)) return valInt64;
                if (dynamicObjectAsJsonElement.TryGetDouble(out var valDouble)) return valDouble;
            ...
            case JsonValueKind.String:
            {
                try
                {
                    ...
                    if (dynamicObjectAsJsonElement.TryGetDateTime(out var dateValue)) return dateValue;
            ...
            case JsonValueKind.Object:
            {
                ...
                while (dynamicObjectPropEnumerator.MoveNext())
                    keyValueObject.Add(dynamicObjectPropEnumerator.Current.Name,
    ...
    public override object Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            if (reader.TryGetInt32(out var valInt32)) return valInt32;

            if (reader.TryGetInt64(out var valInt64)) return valInt64;

            if (reader.TryGetDouble(out var valDouble)) return valDouble;
        }

        if (type == JsonTokenType.String) return reader.GetString();

        if (type is JsonTokenType.True or JsonTokenType.False) return reader.GetBoolean();
