...
namespace UMS.Platform.Common.JsonSerialization.Converters.Helpers;
...
public static class PlatformStringToDateTimeConverterHelper
{
    ...
    public static DateTime? TryRead(string dateTimeStr)
    {
        ...
        try
        {
            ...
            return dateTimeStr.StartsWith('"')
                ? JsonSerializer.Deserialize<DateTime>(dateTimeStr)
                : JsonSerializer.Deserialize<DateTime>($"\"{dateTimeStr}\"");
        }
        catch (Exception)
        {
    ...
    public static DateOnly? TryReadDateOnly(string datetimeOrDateOnlyStr)
    {
        ...
        try
        {
            try
            {
                return DateTime.ParseExact(datetimeOrDateOnlyStr, SupportDateOnlyFormats, CultureInfo.InvariantCulture)
                    .ToDateOnly();
            }
            catch (Exception)
            {
                return DateTime.Parse(datetimeOrDateOnlyStr!, DateTimeFormatInfo.InvariantInfo, DateTimeStyles.None)
                    .ToDateOnly();
            }
        }
        catch (Exception)
