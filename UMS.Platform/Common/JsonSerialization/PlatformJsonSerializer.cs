...
using UMS.Platform.Common.JsonSerialization.Converters;

namespace UMS.Platform.Common.JsonSerialization;
...
public static class PlatformJsonSerializer
{
    ...
    /// </summary>
    public static JsonSerializerOptions ConfigOptions(JsonSerializerOptions options,
        ...
        List<JsonConverter> customConverters = null)
    {
        ...
        if (useCamelCaseNaming) options.PropertyNamingPolicy = JsonNamingPolicy.CamelCase;

        options.Converters.Clear();
        if (useJsonStringEnumConverter) options.Converters.Add(new JsonStringEnumConverter());
    ...
    public static JsonSerializerOptions BuildDefaultOptions(bool useJsonStringEnumConverter = true,
        bool useCamelCaseNaming = false,
        List<JsonConverter> customConverters = null)
    {
        return ConfigOptions(new JsonSerializerOptions(), useJsonStringEnumConverter, useCamelCaseNaming,
            customConverters);
    }

    public static string Serialize<TValue>(TValue value, bool forceUseRuntimeType)
    {
        return Serialize(value, null, forceUseRuntimeType);
    }
    ...
    {
        return Serialize(value, CurrentOptions.Value.Clone().With(customSerializerOptionsConfig));
    }

    public static string SerializeWithDefaultOptions<TValue>(TValue value,
        bool useJsonStringEnumConverter = true,
        bool useCamelCaseNaming = false,
        List<JsonConverter> customConverters = null)
    {
    ...
    public static T DeserializeWithDefaultOptions<T>(string jsonValue,
        bool useJsonStringEnumConverter = true,
        bool useCamelCaseNaming = false,
        List<JsonConverter> customConverters = null)
    {
        return Deserialize<T>(jsonValue,
            BuildDefaultOptions(useJsonStringEnumConverter, useCamelCaseNaming, customConverters));
    }
    ...
    public static object Deserialize(string jsonValue,
        Type returnType,
        ...
        bool forceUseRuntimeType = false)
    {
        if (typeof(TValue).IsAbstract || forceUseRuntimeType)
            ...
            {
                return JsonSerializer.SerializeToUtf8Bytes(value, typeof(TValue),
                    customSerializerOptions ?? CurrentOptions.Value);
            }
    ...
    {
        var tryDeserializeResult = TryDeserialize(json,
            typeof(T),
    ...
    public static bool TryDeserialize(string json,
