...
{
    /// <summary>
    ///     A Command is an imperative instruction to do something; it only has one handler. We will throw an error for
    ///     multiple registered handlers of a command.
    /// </summary>
    ...
    ///     multiple registered handlers of a command.
    /// </summary>
    Task<TResult> SendCommand<TResult>(PlatformCqrsCommand<TResult> command,
        CancellationToken cancellationToken = default)
        where TResult : PlatformCqrsCommandResult, new();
    ...
    ///     To get data by conditions defined in query object.
    /// </summary>
    Task<TResult> SendQuery<TResult>(PlatformCqrsQuery<TResult> query,
        CancellationToken cancellationToken = default);

    /// <summary>
    ///     An Event is a notification that something has happened; it has zero or more handlers.
    /// </summary>
    Task SendEvent(PlatformCqrsEvent cqrsEvent,
        CancellationToken cancellationToken = default);

    /// <summary>
    ///     Send multiple events.
    /// </summary>
    Task SendEvents(IEnumerable<PlatformCqrsEvent> cqrsEvents,
        CancellationToken cancellationToken = default);
}
...
public class PlatformCqrs : IPlatformCqrs
{
    ...
    public Task<TResult> SendCommand<TResult>(PlatformCqrsCommand<TResult> command,
    ...
    public Task<TResult> SendQuery<TResult>(PlatformCqrsQuery<TResult> query,
        CancellationToken cancellationToken = default)
    {
        return mediator.Send(query, cancellationToken);
    }
    ...
    public Task SendEvents(IEnumerable<PlatformCqrsEvent> cqrsEvents,
        CancellationToken cancellationToken = default)
    {
