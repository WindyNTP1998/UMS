...
namespace UMS.Platform.Common.Cqrs.Events;
    ...
    where TEvent : IPlatformCqrsEvent, new()
{
        ...
        bool couldRunInBackgroundThread)
    {
        ...
        if (AllowHandleInBackgroundThread(@event) &&
            ...
            Util.TaskRunner.QueueActionInBackground(async () => await DoExecuteInstanceInNewScope(@event),
    ...
    protected async Task DoExecuteInstanceInNewScope(TEvent notification)
    {
    ...
    protected async Task ExecuteRetryHandleAsync(PlatformCqrsEventHandler<TEvent> handlerNewInstance,
        TEvent notification)
    {
        try
        {
    ...
    protected virtual void CopyPropertiesToNewInstanceBeforeExecution(PlatformCqrsEventHandler<TEvent> previousInstance,
        PlatformCqrsEventHandler<TEvent> newInstance)
    {
        newInstance.ForceCurrentInstanceHandleInCurrentThread =
            previousInstance.ForceCurrentInstanceHandleInCurrentThread;
    }
    ...
    protected async Task ExecuteHandleWithTracingAsync(TEvent @event, Func<Task> handleAsync)
    {
        if (IsDistributedTracingEnabled)
            using (var activity =
                   IPlatformCqrsEventHandler.ActivitySource.StartActivity($"EventHandler.{nameof(ExecuteHandleAsync)}"))
            {
