...
/// <summary>
///     Use this property on Entity and on Property you want to check to auto add
///     <see cref="ISupportDomainEventsEntity.FieldUpdatedDomainEvent" /> on update property one the entity.
        ...
        where TEntity : class, IEntity, new()
    {
        if (entity.HasTrackValueUpdatedDomainEventAttribute())
            typeof(TEntity)
                ...
                .Where(p => p.Name != nameof(IRowVersionEntity.ConcurrencyUpdateToken) &&
                            p.Name != nameof(IDateAuditedEntity.LastUpdatedDate))
                ...
                .Where(propertyInfo => propertyInfo.GetValue(entity)
                    .IsValuesDifferent(propertyInfo.GetValue(existingOriginalEntity)))
                .ForEach(propertyInfo =>
                {
                    entity.As<ISupportDomainEventsEntity<TEntity>>()
                        .AddFieldUpdatedEvent(propertyInfo,
                            propertyInfo.GetValue(existingOriginalEntity),
