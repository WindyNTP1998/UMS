...
public static class WithDomainExceptionValidationExtension
{
    ...
    public static async Task<PlatformValidationResult<T>> WithDomainException<T>(
    ...
    public static async Task<PlatformValidationResult<T>> WithDomainValidationException<T>(
        this Task<PlatformValidationResult<T>> valTask)
    {
        var applicationVal = await valTask;
