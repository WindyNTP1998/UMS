...
namespace UMS.Platform.Domain.Exceptions.Extensions;
...
public static class EnsureThrowDomainExceptionExtension
{
    ...
    public static async Task<T> EnsureDomainValidationValid<T>(this Task<T> valueTask, Func<T, bool> must,
        string errorMsg)
    {
    ...
    {
        if (validationResult is { IsValid: false }) throw new PlatformDomainValidationException(validationResult);
    }
    ...
    public static void EnsureDomainValidationValid(this List<Func<PlatformValidationResult>> validationResultFns)
    {
        validationResultFns.ForEach(validationResultFn =>
        {
            ...
            if (validationResult?.IsValid == false) throw new PlatformDomainValidationException(validationResult);
        });
    }
    ...
    public static async Task EnsureDomainValidationValid(
        this List<Func<Task<PlatformValidationResult>>> validateActions)
    {
    ...
    public static async Task EnsureEntityValid<TEntity, TPrimaryKey>(this TEntity entity,
        ...
        where TEntity : IEntity<TPrimaryKey>
    {
        ...
        if (entityCheckUniquenessValidator != null)
            await entityCheckUniquenessValidator.Validate(predicate => anyAsyncFunc(predicate, cancellationToken))
                .EnsureDomainValidationValid();
    }

    public static async Task EnsureEntitiesValid<TEntity, TPrimaryKey>(this List<TEntity> entities,
        Func<Expression<Func<TEntity, bool>>, CancellationToken, Task<bool>> anyAsyncFunc,
        CancellationToken cancellationToken) where TEntity : IEntity<TPrimaryKey>
    {
    ...
    public static async Task EnsureEntitiesUnique<TEntity, TPrimaryKey>(this List<TEntity> entities,
        Func<Expression<Func<TEntity, bool>>, CancellationToken, Task<bool>> anyAsyncFunc,
        CancellationToken cancellationToken) where TEntity : IEntity<TPrimaryKey>
    {
        ...
        var validateEntityUniqueActions = entities
            ...
            .Select<IValidatableEntity<TEntity, TPrimaryKey>, Func<Task<PlatformValidationResult>>>(entity =>
                () => entity.CheckUniqueValidator()
                    .Validate(async findOtherDuplicatedItemPredicate =>
