...
public abstract class PlatformCqrsEntityEvent : PlatformCqrsEvent, IPlatformUowEvent, IPlatformCqrsEntityEvent
{
    ...
    private static async Task SendEvent<TEvent>(IServiceProvider serviceProvider,
        ...
        where TEvent : PlatformCqrsEntityEvent
    {
        ...
        if (mappedToDbContextUow != null)
            await mappedToDbContextUow.CreatedByUnitOfWorkManager.CurrentSameScopeCqrs.SendEvent(entityEvent,
                cancellationToken);
        else
            await serviceProvider.ExecuteInjectScopedAsync((IPlatformCqrs cqrs) =>
            {
                cqrs.SendEvent(entityEvent, cancellationToken);
            });
    }

    public static async Task SendEvent<TEntity, TPrimaryKey>(IServiceProvider serviceProvider,
        [AllowNull] IUnitOfWork mappedToDbContextUow,
        TEntity entity,
    ...
    {
        await SendEvent(serviceProvider,
    ...
    public static async Task SendBulkEntitiesEvent<TEntity, TPrimaryKey>(IServiceProvider serviceProvider,
        [AllowNull] IUnitOfWork mappedToDbContextUow,
        ...
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        var result = await deleteEntityAction(entity)
            ...
            {
                if (!dismissSendEvent)
                    await SendEvent<TEntity, TPrimaryKey>(serviceProvider,
                        mappedToDbContextUow,
                        entity,
    ...
    public static async Task<TResult> ExecuteWithSendingUpdateEntityEvent<TEntity, TPrimaryKey, TResult>(
        ...
        CancellationToken cancellationToken = default) where TEntity : class, IEntity<TPrimaryKey>, new()
    {
        if (!dismissSendEvent) entity.AutoAddFieldUpdatedEvent(existingEntity);
...
///     Implement and <see cref="Application.Cqrs.Events.PlatformCqrsEventApplicationHandler{TEvent}" /> to handle any
///     events.
/// </summary>
    ...
    where TEntity : class, IEntity, new()
{
    ...
    public PlatformCqrsEntityEvent(TEntity entityData,
        PlatformCqrsEntityEventCrudAction crudAction)
    {
    ...
    {
        return DomainEvents
            .Where(p => p.Key == ISupportDomainEventsEntity.DomainEvent
                .GetDefaultEventName<ISupportDomainEventsEntity.FieldUpdatedDomainEvent>())
            .Select(p =>
                PlatformJsonSerializer
                    .TryDeserializeOrDefault<ISupportDomainEventsEntity.FieldUpdatedDomainEvent<TValue>>(p.Value))
            .FirstOrDefault(p => p != null && p.FieldName == field.GetPropertyName());
    }

    public bool HasAnyFieldUpdated(params Expression<Func<TEntity, object>>[] fields)
    {
        ...
        return DomainEvents
            ...
            .Select(p =>
                PlatformJsonSerializer
                    .TryDeserializeOrDefault<ISupportDomainEventsEntity.FieldUpdatedDomainEvent<object>>(p.Value))
    ...
    where TEntity : class, IEntity<TPrimaryKey>, new()
{
    ...
    public PlatformCqrsBulkEntitiesEvent(IList<TEntity> entities,
        PlatformCqrsEntityEventCrudAction crudAction)
    {
        AuditTrackId = Guid.NewGuid().ToString();
        Entities = entities;
        ...
        if (typeof(TEntity).IsAssignableTo(typeof(ISupportDomainEventsEntity)))
            DomainEvents = entities.ToDictionary(entity => entity.Id,
    ...
    public bool HasAnyFieldUpdated(TPrimaryKey entityId,
